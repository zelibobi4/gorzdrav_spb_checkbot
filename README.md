...
Бот периодически проверяет доступность талончиков к врачу и отправляет уведомление в Telegram, если найден подходящий свободный талон. Можно ограничить поиск количеством дней и временем приёма, например ждать только вечерние слоты после 17:00. Также после выбора специальности можно включить режим «любой врач этой специальности» — удобно, например, для стоматологов.
...
`/set_doctor` | выбрать конкретного врача или режим «любой врач этой специальности»
...
- хранит пользователей бота:
  ...
  - режим отслеживания: конкретный врач или любой врач специальности
  - выбранную поликлинику и специальность для режима «любой врач»
...
- fork: добавлены фильтр времени приёма (`/evening`, `/time`, `/time_off`) и режим «любой врач этой специальности»
